...
這是一個基於 Go (Golang) 開發的場地預約系統後端 API。專案採用 **Clean Architecture (分層架構)** 設計，具備完整的身份驗證 (JWT)、角色權限管理 (RBAC) 以及組織/場地管理功能。
...
本專案遵循 **分層架構 (Layered Architecture)**，將關注點分離，確保程式碼的可測試性與維護性。依賴關係由外向內（Handler -\> Service -\> Repository），並透過 `internal/app/container.go` 進行 **依賴注入 (Dependency Injection)**。
...
2.  **設定環境變數**
    ...
    確保 `.env` 內容包含正確的 DB 連線字串（預設對應 `compose.yml`）：
...
3.  **啟動資料庫與 Swagger**
    ...
    _這會自動執行 `db/schema.sql` 初始化資料庫表格。_
